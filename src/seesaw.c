...
bool seesaw_read(uint8_t addr, uint8_t module, uint8_t reg,
    ...
    // Write the address/register we want to read from. 
    // CHANGED: nostop = false. We send a stop bit.
    if (i2c_write_blocking(NEOTRELLIS_I2C, addr, hdr, 2, false) < 0) return false;

    // Wait a bit longer for the chip to process
    sleep_us(600); 
